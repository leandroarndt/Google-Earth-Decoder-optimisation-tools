...
def install_python_lib(lib):
    ...
    {python} -m pip install {lib}""".format(python=bpy.app.binary_path_python, lib=lib)
    success_msg = "pip and " + lib + " correctly installed in blender lib folder"

    if importlib.util.find_spec(lib) is None:
        try:
            # install or upgrade pip
            subprocess.run([bpy.app.binary_path_python, "-m", "ensurepip", '--upgrade'], shell=True, check=True)

            # install required packages
            subprocess.run([bpy.app.binary_path_python, "-m", "pip", "install", "--upgrade", lib], shell=True, check=True)
        except:
